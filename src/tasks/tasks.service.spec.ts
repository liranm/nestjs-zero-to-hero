...
describe('TasksService', () => {
  ...
  beforeEach(async () => {
    const module = await Test.createTestingModule({
      providers: [
        TasksService,
        { provide: TaskRepsitory, useFactory: mockTaskRepository },
      ],
    }).compile();

    tasksService = await module.get<TasksService>(TasksService);
    taskRepository = await module.get<TaskRepsitory>(TaskRepsitory);
  });
  ...
  describe('deleteTask', () => {
    it('calls taskRepository.delete() to delete a task', async () => {
      ...
      expect(taskRepository.delete).toHaveBeenLastCalledWith({ id: 1, userId: mockUser.id });
    });
